...
async function createUser() {
  try {
    ...
    const browserName = (await question('Enter browser name (Optional): ')).trim() || null;
    ...
    if (existingUser) {
      ...
      const updateChoice = await question('Do you want to generate a new token for this user? (y/n): ');
